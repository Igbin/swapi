...
fetch(URL)
  ...
  .then(text => {
    ...
    X.onclick = function(e) {
      ...
      alertMsg.innerHTML = `Are you sure that you want to delete ${
      ...
      document.getElementById("cancel").addEventListener("click", () => {
        for (let i = 0; i < planetArr.length; i++) {
          for (let key in planetArr[i]) {
            if (
              planetArr[i][key] ===
              thisPlanetname.substring(0, thisPlanetname.length - 1)
            ) {
        ...
        for (key in elem) {
          ...
          if (key == "url") {
            ...
            for (let i = 0; i < elem[key].length; i++) {
              td2.innerHTML += `<a href='${elem[key][i]}'>${
                elem[key][i]
              }</a><br>`;
            }
    ...
    function myUpdate(e) {
      ...
      for (let i = 0; i < event.target.parentNode.children.length; i++) {
        if (
          event.target.parentNode.children[i].innerHTML ===
      ...
      input.addEventListener("blur", e => {
        ...
        if (
          !isNaN(parseInt(input.parentNode.textContent)) &&
          ...
          input.parentNode.textContent ===
            input.parentNode.parentNode.firstChild.textContent
        ) {
          this.innerHTML =
            event.target.value + "<br><span style='color:red'>edited</span>";
          this.appendChild(X);
        ...
        for (let i = 0; i < planetArr.length; i++) {
          for (let key in planetArr[i]) {
            if (planetArr[i].name === data) {
              planetArr[i][objProp] =
                event.target.value +
                "<br><span style='color:red'>edited</span>";
              return;
            }
          }
        }
      ...
      for (let i = 0; i < headLine.length; i++) {
        ...
        function checkRows() {
          if (input.value == "") {
            event.target.focus();
            alertMsg.innerHTML =
              "Enter the  " + headLine[i].textContent + " of planet!";
            modal.classList.remove("hidden");
            ...
            for (let key in newPlanet) {
              if (headLine[i].textContent == key) newPlanet[key] = input.value;
            }
          }
          if (result.querySelector("input")) {
